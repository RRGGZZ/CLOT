...
def determine_obs_dim(config) -> None:
    ...
    config.robot.history_num = config.env.config.obs.history_len
    config.robot.future_num = config.env.config.obs.future_ref_steps
    if 'history_len_amp' in config.env.config.obs:
        config.robot.history_num_amp = config.env.config.obs.history_len_amp
...
def pre_process_config(config) -> None:
    ...
    # # Resolve the "depend on algo" placeholder for use_transformer
