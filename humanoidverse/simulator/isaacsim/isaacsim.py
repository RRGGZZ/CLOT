...
import inspect
import copy
from humanoidverse.simulator.isaacsim.isaacsim_articulation_cfg import ARTICULATION_CFG, ADAM_SP_ACTION_SCALE
...
from humanoidverse.simulator.isaacsim.events import randomize_body_com, randomize_rigid_body_mass, randomize_joint_friction, randomize_rigid_body_restitution
...
class IsaacSim(BaseSimulator):
    def __init__(self, config, device):
        ...
        scene_config: InteractiveSceneCfg = InteractiveSceneCfg(num_envs=self.num_envs, env_spacing=self.simulator_config.scene.env_spacing, replicate_physics=self.simulator_config.scene.replicate_physics)
        # generate scene
        ...
        if builtins.ISAAC_LAUNCHED_FROM_TERMINAL is False:
            ...
            with Timer("[INFO]: Time taken for simulation start", "simulation_start"):
                self.sim.reset()
        # Initialize domain randomization related attributes
        ...
        if "randomize_link_mass" in self.domain_rand_config:
        ...
        # Randomize joint friction (使用原生isaaclab函数)
        ...
        if self.domain_rand_config.get("randomize_pd_gain", False):
            self.events_cfg.randomize_pd_gain = EventTerm(
                func=mdp.randomize_actuator_gains,
                mode="startup",
                params={
                    "asset_cfg": SceneEntityCfg("robot", joint_names=".*"),
                    "stiffness_distribution_params": tuple(self.domain_rand_config["kp_range"]),
                    "damping_distribution_params": tuple(self.domain_rand_config["kd_range"]),
                    "operation": "scale",
    ...
    def _read_friction_coeffs_from_physics(self):
        """从物理引擎读取实际的 friction 系数并保存到 friction_coeffs
        
        仅在使用原生 mdp 函数时需要调用此方法
        """
        # 使用正确的 API 方法名称：get_dof_friction_coefficients
    ...
    def _read_restitution_coeffs_from_physics(self):
        """从物理引擎读取实际的 restitution 系数并保存到 restitution_coeffs
    ...
    def load_assets(self):
        ...
        # print(self.dof_ids, self.dof_names)
