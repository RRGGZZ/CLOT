...
class LeggedRobotBase(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.base_quat = self.simulator.base_quat # XYZW  isaacsim 内部转换过了
        ...
        self.last_last_actions = torch.zeros(self.num_envs, self.dim_actions, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        self.effort_limit = torch.tensor(self.config.robot.dof_effort_limit_list, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        self.action_scale = 0.25 * self.effort_limit / self.p_gains
        ...
        self.average_end_time_ratio = 0. # num_compute_average_epl last termination episode length
        ...
        self.track_motion_end_ratio = torch.zeros(self.num_envs, device=self.device, dtype=torch.long)
        self.reset_buf_track = torch.zeros(self.num_envs, device=self.device, dtype=torch.long)
        ...
        self.motion_advance_buffer = torch.zeros(self.num_envs, dtype=torch.long, device=self.device, requires_grad=False)
        # if self.config.simulator.config.name == 'mjlab':
        #     # from humanoidverse.asset_zoo.robots import G1_ACTION_SCALE
        #     # import ipdb; ipdb.set_trace()
        #     from humanoidverse.asset_zoo.robots import ( ADAM_SP_ACTION_SCALE, get_adam_sp_robot_cfg,)
        #     for i, dof_name in enumerate(self.dof_names):
        #         action_scale = self.get_action_scale_for_joint(dof_name, ADAM_SP_ACTION_SCALE)
        #         if action_scale is not None:
        #             self.action_scale[i] = action_scale
        #         else:
        #             logger.warning(f"Action scale for joint {dof_name} not found in ADAM_SP_ACTION_SCALE. Available patterns: {list(ADAM_SP_ACTION_SCALE.keys())}")
        #             raise ValueError(f"Action scale for joint {dof_name} not found in ADAM_SP_ACTION_SCALE")
        #     logger.info(f"action_scale initialized: {self.action_scale}")
        #     logger.info(f"ADAM_SP_ACTION_SCALE: {ADAM_SP_ACTION_SCALE}")
        #     # 断言检查 action_scale 是否等于 0.25 * effort_limit / p_gains
        #     expected_action_scale = 0.25 * self.effort_limit / self.p_gains
            # assert torch.allclose(self.action_scale, expected_action_scale, rtol=1e-5, atol=1e-8), \
            #     f"action_scale mismatch: got {self.action_scale}, expected {expected_action_scale}"
        
        # if self.config.simulator.config.name == 'isaacsim':
        #     from humanoidverse.simulator.isaacsim.isaacsim_articulation_cfg import ADAM_SP_ACTION_SCALE
        #     for i, dof_name in enumerate(self.dof_names):
        #         action_scale = self.get_action_scale_for_joint(dof_name, ADAM_SP_ACTION_SCALE)
        #         if action_scale is not None:
        #             self.action_scale[i] = action_scale
    ...
    def _domain_rand_config(self):
        if self.config.domain_rand.push_robots:
            self.push_interval_s = torch.randint(self.config.domain_rand.push_interval_s[0], self.config.domain_rand.push_interval_s[1], (self.num_envs,), device=self.device, dtype=torch.float)
        
        if self.config.domain_rand.motion_time_advance:
            self.advance_interval_s = torch.randint(self.config.domain_rand.advance_interval_s[0], self.config.domain_rand.advance_interval_s[1], (self.num_envs,), device=self.device, dtype=torch.float)


    def _init_counters(self):
        self.common_step_counter = 0
        self.push_robot_counter = torch.zeros(self.num_envs, dtype=torch.int, device=self.device, requires_grad=False)
        self.appaly_advance_counter = torch.zeros(self.num_envs, dtype=torch.int, device=self.device, requires_grad=False)
    ...
    def _init_domain_rand_buffers(self):
        ...
        if self.config.domain_rand.born_offset:
            self.born_distance = self.config.domain_rand.born_distance
        if self.config.domain_rand.born_heading:
            self.born_heading_degree = self.config.domain_rand.born_heading_degree
        if self.config.domain_rand.push_robots:
            self.push_robots_interval = self.config.domain_rand.push_robots_interval
        if self.config.domain_rand.motion_time_advance:
            self.advance_initial_degree = self.config.domain_rand.advance_initial_degree
            self.appaly_advance_interval = self.config.domain_rand.appaly_advance_interval
    ...
    def step(self, actor_state):
        ...
        if 'use_transformer' in self.config and self.config.use_transformer:
            return self.transformer_obs_buf_dict, self.rew_buf, self.reset_buf, self.extras
        else:
    ...
    def _apply_force_in_physics_step(self):
        if self.config.simulator.config.name == 'mjlab':
            self.joint_target = self.actions_after_delay * self.action_scale + self.default_dof_pos
            # self.torques = self.actions_after_delay.view(self.torques.shape)
            self.simulator.apply_actions(self.joint_target)
    ...
    def _post_physics_step(self):
        ...
        if self.config.domain_rand.motion_time_advance:
            self.motion_advance_buffer = (self.motion_advance_buffer - 1).clamp_min(0)
        # update counters
        ...
        if 'use_transformer' in self.config and self.config.use_transformer:
            for obs_key, obs_val in self.transformer_obs_buf_dict.items():
                self.transformer_obs_buf_dict[obs_key] = torch.clip(obs_val, -clip_obs, clip_obs)
        else:
    ...
    def _pre_compute_observations_callback(self):
        ...
        self.base_ang_vel[:] = quat_rotate_inverse(self.base_quat, self.simulator.robot_root_states[:, 10:13])
        self.projected_gravity[:] = quat_rotate_inverse(self.base_quat, self.gravity_vec)
        self.contacts = ( self.simulator.contact_forces[:, self.feet_indices, 2] > 1.).float()
        # print(f"self.contacts: {self.contacts}")
    ...
    def _update_tasks_callback(self):
        if self.config.domain_rand.push_robots:
            ...
            if len(push_robot_env_ids) > 0:
                self.push_interval_s[push_robot_env_ids] = torch.tensor(self.push_robots_interval, device=self.device, requires_grad=False, dtype=torch.float) + torch.randint(self.config.domain_rand.push_interval_s[0], self.config.domain_rand.push_interval_s[1], (len(push_robot_env_ids),), device=self.device, requires_grad=False, dtype=torch.float)
        ...
        if self.config.domain_rand.motion_time_advance and self.config.domain_rand.appaly_advance_interval:
            apply_advance_env_ids = (self.appaly_advance_counter == (self.advance_interval_s / self.dt).int()).nonzero(as_tuple=False).flatten()
            self.appaly_advance_counter[apply_advance_env_ids] = 0
            if len(apply_advance_env_ids) > 0:
                self.advance_interval_s[apply_advance_env_ids] = torch.tensor(self.appaly_advance_interval, device=self.device, requires_grad=False, dtype=torch.float) + torch.randint(self.config.domain_rand.advance_interval_s[0], self.config.domain_rand.advance_interval_s[1], (len(apply_advance_env_ids),), device=self.device, requires_grad=False, dtype=torch.float)
                motion_advance_time = torch_rand_float(0.0, self.advance_initial_degree, (self.num_envs, 1), device=self.device).squeeze(-1)
                self.motion_advance_buffer[apply_advance_env_ids] = (motion_advance_time[apply_advance_env_ids] // self.dt).long()
    ...
    def _check_termination(self):
        ...
        self.reset_buf_track += self.reset_buf
        env_ids = self.reset_buf.nonzero(as_tuple=False).flatten()
        self.log_dict["success_rate"] = self.track_motion_end_ratio.float().mean() / (self.reset_buf_track.float().mean() + 1e-15)
        if len(env_ids) > 0:
            for key in self.reset_buf_terminate_by.keys():
                self.log_dict[f"terminate_by_{key}"] = self.reset_buf_terminate_by[key][env_ids].float().mean()
            for key in self.log_dict.keys():
                if key.startswith("terminate_by_"):
                    self.log_dict[key] = self.log_dict[key] / (self.reset_buf[env_ids].float().mean() + 1e-15)
    ...
    def _reset_tasks_callback(self, env_ids):
        ...
        if self.config.domain_rand.born_offset_curriculum:
            self._update_born_offset_curriculum()
        if self.config.domain_rand.born_heading_curriculum:
            self._update_born_heading_curriculum()
        if self.config.domain_rand.push_robots_interval_curriculum:
            self._update_push_robots_interval_curriculum()
        if self.config.domain_rand.motion_time_advance:
            self._update_motion_time_advance_curriculum()
            self._update_appaly_advance_curriculum()
        if self.use_noise_process:
    ...
    def _update_obs_noise_curriculum(self):
        ...
        if self.config.use_end_time_ratio:
            self.current_noise_curriculum_value = self.average_end_time_ratio * self.config.obs.noise_value_max + (1-self.average_end_time_ratio) * self.config.obs.noise_value_min
    ...
    def _reset_buffers_callback(self, env_ids, target_buf=None):
        if target_buf is not None:
            ...
            self.last_last_actions[env_ids] = target_buf["last_last_actions"].to(self.last_last_actions.dtype)
    ...
    def get_mppi_buffers(self, env_ids):
        ...
        return {
            ...
            "last_last_actions": copy.deepcopy(self.last_last_actions[env_ids]),
    ...
    def _post_config_observation_callback(self):
        ...
        self.transformer_obs_buf_dict = dict()
        
        for obs_key, obs_config in self.config.obs.obs_dict.items():
            obs_keys = sorted(obs_config)
            # logger.info('obs_keys: {}', obs_keys)
            # print("obs_keys", obs_keys)
            # import ipdb; ipdb.set_trace()
            if 'use_transformer' in self.config and self.config.use_transformer:
                if 'amp' in obs_key:
                    self.transformer_obs_buf_dict[obs_key] = torch.cat([self.obs_buf_dict_raw[obs_key][key] for key in obs_keys], dim=-1)
                else:
                    self.transformer_obs_buf_dict[obs_key+'_current'] = torch.cat([self.obs_buf_dict_raw[obs_key][key] for key in obs_keys if 'history' not in key and 'future' not in key], dim=-1).unsqueeze(1)
                    self.transformer_obs_buf_dict[obs_key+'_past'] = torch.cat([self.obs_buf_dict_raw[obs_key][key] for key in obs_keys if 'history' in key], dim=-1)
                    self.transformer_obs_buf_dict[obs_key+'_future'] = torch.cat([self.obs_buf_dict_raw[obs_key][key] for key in obs_keys if 'future' in key], dim=-1)
                # import ipdb; ipdb.set_trace()
            else:
    ...
    def _compute_torques(self, actions):
        ...
        if "action_filt" in self.config.robot.control and self.config.robot.control.action_filt:
            actions = self.action_filter.filter(actions.reshape(self.num_envs * self.dim_actions)).reshape(self.num_envs, self.dim_actions)
        ...
        # logger.info("torques: {} ,\
        #             torques lower dif: {},\
        #             torques upper dif: {}",torques,torques + self.torque_limits,- torques + self.torque_limits)
    ...
    def _update_reward_penalty_curriculum(self):
        ...
        if self.config.use_end_time_ratio:
            self.reward_penalty_scale = self.average_end_time_ratio



    def _update_born_offset_curriculum(self):
        if self.average_episode_length < self.config.domain_rand.born_offset_level_down_threshold:
            self.born_distance *= (1 - self.config.domain_rand.level_degree)
        elif self.average_episode_length > self.config.domain_rand.born_offset_level_up_threshold:
            self.born_distance *= (1 + self.config.domain_rand.level_degree)
        # import ipdb; ipdb.set_trace()
        # torch.clamp(randomize_distance,self.cfg.domain_rand.born_offset_range[0], self.cfg.domain_rand.born_offset_range[1])
        if self.config.use_end_time_ratio:
            self.born_distance = self.average_end_time_ratio * self.config.domain_rand.born_offset_range[1]
    ...
    def _update_born_heading_curriculum(self):
        if self.average_episode_length < self.config.domain_rand.born_heading_level_down_threshold:
            self.born_heading_degree *= (1 - self.config.domain_rand.born_heading_level_degree)
        elif self.average_episode_length > self.config.domain_rand.born_heading_level_up_threshold:
            self.born_heading_degree *= (1 + self.config.domain_rand.born_heading_level_degree)
        # import ipdb; ipdb.set_trace()
        # torch.clamp(randomize_distance,self.cfg.domain_rand.born_offset_range[0], self.cfg.domain_rand.born_offset_range[1])
        if self.config.use_end_time_ratio:
            self.born_heading_degree = self.average_end_time_ratio * self.config.domain_rand.born_heading_range[1]
    ...
    def _update_push_robots_interval_curriculum(self):
        if self.average_episode_length < self.config.domain_rand.push_robots_level_down_threshold:
            self.push_robots_interval *= (1 + self.config.domain_rand.push_robots_level_degree)
        elif self.average_episode_length > self.config.domain_rand.push_robots_level_up_threshold:
            self.push_robots_interval *= (1 - self.config.domain_rand.push_robots_level_degree)
        # import ipdb; ipdb.set_trace()
        # torch.clamp(randomize_distance,self.cfg.domain_rand.born_offset_range[0], self.cfg.domain_rand.born_offset_range[1])
        if self.config.use_end_time_ratio:
            self.push_robots_interval = self.average_end_time_ratio * self.config.domain_rand.push_robots_range[0] + (1-self.average_end_time_ratio) * self.config.domain_rand.push_robots_range[1]
    ...
    def _update_appaly_advance_curriculum(self):
        if self.average_episode_length < self.config.domain_rand.advance_level_down_threshold:
            self.appaly_advance_interval *= (1 + self.config.domain_rand.advance_degree)
        elif self.average_episode_length > self.config.domain_rand.advance_level_up_threshold:
            self.appaly_advance_interval *= (1 - self.config.domain_rand.advance_degree)
        if self.config.use_end_time_ratio:
            self.appaly_advance_interval = self.average_end_time_ratio * self.config.domain_rand.appaly_advance_range[0] + (1-self.average_end_time_ratio) * self.config.domain_rand.appaly_advance_range[1]
    ...
    def _update_motion_time_advance_curriculum(self):
        if self.average_episode_length < self.config.domain_rand.advance_level_down_threshold:
            ...
            self.advance_initial_degree *= (1 + self.config.domain_rand.advance_degree)
        if self.config.use_end_time_ratio:
            self.advance_initial_degree = self.average_end_time_ratio * self.config.domain_rand.advance_range[1]
    ...
    def _update_reward_limits_curriculum(self):
        ...
        if self.use_reward_limits_dof_pos_curriculum:
            if self.average_episode_length < curriculum_cfg.soft_dof_pos_curriculum_level_down_threshold:
                ...
                self.soft_dof_pos_curriculum_value *= (1 - curriculum_cfg.soft_dof_pos_curriculum_degree)
            if self.config.use_end_time_ratio:
                self.soft_dof_pos_curriculum_value = self.average_end_time_ratio * self.config.rewards.reward_limit.reward_limits_curriculum.soft_dof_pos_min_limit + (1-self.average_end_time_ratio) * self.config.rewards.reward_limit.reward_limits_curriculum.soft_dof_pos_max_limit
        ...
        if self.use_reward_limits_dof_vel_curriculum:
            if self.average_episode_length < curriculum_cfg.soft_dof_vel_curriculum_level_down_threshold:
                ...
                self.soft_dof_vel_curriculum_value *= (1 - curriculum_cfg.soft_dof_vel_curriculum_degree)
            if self.config.use_end_time_ratio:
                self.soft_dof_vel_curriculum_value = self.average_end_time_ratio * self.config.rewards.reward_limit.reward_limits_curriculum.soft_dof_vel_min_limit + (1-self.average_end_time_ratio) * self.config.rewards.reward_limit.reward_limits_curriculum.soft_dof_vel_max_limit
        ...
        if self.use_reward_limits_torque_curriculum:
            if self.average_episode_length < curriculum_cfg.soft_torque_curriculum_level_down_threshold:
                ...
                self.soft_torque_curriculum_value *= (1 - curriculum_cfg.soft_torque_curriculum_degree)
            if self.config.use_end_time_ratio:
                self.soft_torque_curriculum_value = self.average_end_time_ratio * self.config.rewards.reward_limit.reward_limits_curriculum.soft_torque_min_limit + (1-self.average_end_time_ratio) * self.config.rewards.reward_limit.reward_limits_curriculum.soft_torque_max_limit
    ...
    def _reward_gravity_termination(self):
        # Terminal reward / penalty
        return self.reset_buf_terminate_by["gravity"]

    def _reward_alive(self):
        # Reward alive
        return 1.0 - 1.0 * (self.reset_buf * ~self.time_out_buf)
    ...
    def _reward_penalty_action_smoothness(self):
        # Penalize changes in actions
        return torch.sum(torch.square((self.last_last_actions - 2*self.last_actions + self.actions)/2), dim=1)
    ...
    def _reward_penalty_torque_outof_limits(self):
        if self.use_reward_limits_dof_pos_curriculum:
            m = (self.simulator.hard_dof_pos_limits[:, 0] + self.simulator.hard_dof_pos_limits[:, 1]) / 2
            r = self.simulator.hard_dof_pos_limits[:, 1] - self.simulator.hard_dof_pos_limits[:, 0]
            lower_soft_limit = m - 0.5 * r * self.soft_dof_pos_curriculum_value
        ...
        below_lower_limit = (self.simulator.dof_pos < lower_soft_limit).float()
        above_upper_limit = (self.simulator.dof_pos > upper_soft_limit).float()
        
        penalty_below = below_lower_limit * torch.clamp(self.torques, max=0.0) ** 2
        penalty_above = above_upper_limit * torch.clamp(self.torques, min=0.0) ** 2
        
        total_penalty = torch.sum(penalty_below + penalty_above, dim=1)
    ...
    def _reward_limits_dof_vel(self):
        ...
        # # logger.info("dof_vel: {} ,\
        # #             dof_vel lower dif: {},\
        # #             dof_vel upper dif: {}",self.simulator.dof_vel,self.simulator.dof_vel + self.dof_vel_limits,- self.simulator.dof_vel + self.dof_vel_limits)
        # out_of_limits = -torch.log(torch.clamp(self.simulator.dof_vel + self.dof_vel_limits,min=1e-3,max=1))-torch.log(torch.clamp(-(self.simulator.dof_vel - self.dof_vel_limits),min=1e-3,max=1))
        # return torch.sum(out_of_limits, dim=1)
    ...
    def _reward_limits_torque(self):
        ...
        # out_of_limits = -torch.log(torch.clamp((self.torques + self.torque_limits)/10,min=1e-3,max=1))-torch.log(torch.clamp(-(self.torques - self.torque_limits)/10,min=1e-3,max=1))
        # return torch.sum(out_of_limits,dim=1)
    ...
    def _reward_feet_min_height_for_this_air(self):
        ...
        try:
            ...
            from_air_to_contact = contact_filt
            logger.warning("from_air_to_contact computation failed, using now contacts")
        # self.last_contacts = contact
        # self.last_contacts_filt = contact_filt

        self.feet_air_min_height = torch.max(self.feet_air_min_height, self.simulator._rigid_body_pos[:, self.feet_indices, 2])
        
        rew_feet_min_height = torch.sum((12 * torch.clamp(self.feet_air_min_height - self.config.rewards.desired_feet_min_height_for_this_air, max=0.0003) + \
                                         12 * torch.clamp(self.config.rewards.desired_feet_max_height_for_this_air - self.feet_air_min_height, max=0.0003)) * from_air_to_contact, dim=1) # reward only on first contact with the ground
    ...
    def get_action_scale_for_joint(self, joint_name: str, action_scale_dict: Dict[str, float]) -> float:
        """
        根据关节名称，从 G1_ACTION_SCALE 字典中获取对应的 action scale 值
        
        Args:
            joint_name: 实际的关节名称，如 'left_elbow_joint'
            action_scale_dict: G1_ACTION_SCALE 字典，键为正则表达式模式
        
        Returns:
            匹配的 action scale 值，如果没有匹配则返回 None
        """
        for pattern, scale in action_scale_dict.items():
            # 使用 re.fullmatch 进行完整匹配
            # 注意：pattern 可能已经是正则表达式字符串，直接使用即可
            try:
                if re.fullmatch(pattern, joint_name):
                    return scale
            except re.error as e:
                # 如果 pattern 不是有效的正则表达式，尝试作为普通字符串匹配
                logger.warning(f"Invalid regex pattern '{pattern}': {e}, trying exact match")
