...
def axis_angle_to_matrix(axis_angle: torch.Tensor) -> torch.Tensor:
    ...
    quat = axis_angle_to_quaternion(axis_angle)  # wxyz format
...
def quaternion_apply(quaternion: torch.Tensor, point: torch.Tensor) -> torch.Tensor:
    ...
    Quaternion format: wxyz (w is the real part)
    
    Args:
        quaternion: Quaternions as tensor of shape (..., 4), wxyz format
        point: Points as tensor of shape (..., 3)
    
    Returns:
        Rotated points as tensor of shape (..., 3)
    """
    ...
    # wxyz format: extract components
    w = quaternion[..., 0:1]
    xyz = quaternion[..., 1:]
    
    # Quaternion rotation formula: v' = v + 2 * w * (xyz x v) + 2 * xyz x (xyz x v)
    t = torch.cross(xyz, point, dim=-1) * 2
    return point + w * t + torch.cross(xyz, t, dim=-1)
...
def quaternion_multiply(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    Args:
        a: Quaternions as tensor of shape (..., 4), wxyz format
        b: Quaternions as tensor of shape (..., 4), wxyz format
    
    Returns:
        Product quaternions as tensor of shape (..., 4)
    """
    if a.shape[-1] != 4 or b.shape[-1] != 4:
        raise ValueError(f"Invalid quaternion shapes {a.shape}, {b.shape}")
