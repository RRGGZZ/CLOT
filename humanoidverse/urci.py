...
def main(override_config: OmegaConf):
        ...
        try:
            if hasattr(key, 'char'):
                if key.char == 'r' or key.char == 'R':
                    ...
                    if hasattr(robot, 'is_recording') and robot.is_recording and hasattr(robot, '_video_buffer') and len(robot._video_buffer) > 0:
                        logger.info("Saving video on R key press...")
                        import time
                        model_path:Path = robot.cfg.checkpoint
                        ...
                        import imageio
                        imageio.mimsave(video_path, robot._video_buffer, fps=robot.fps, macro_block_size=robot.macro_block_size)
    ...
    def listen_for_keypress(robot):
        with keyboard.Listener(on_press=lambda key: on_press(key, robot)) as listener:
            listener.join()
    ...
    key_listener_thread = threading.Thread(target=listen_for_keypress, args=(robot,))
